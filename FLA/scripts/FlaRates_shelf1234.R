#FlaRatesEN556_bulk for hydrolysis rates calculated from bulk water incubations

#labeling scheme: stn#-d#-bulk-[substrate]-[rep]-t#
#e.g. stn7-d0-bulk-lam-x-t3 or stn4-d5-bulk-chon-1-t0

#define path where your scripts are
path <- getwd()
...
outputMaster <- "FlaRates_shelf1234.csv"
#Read in info about which std bins to use for which samples
StdsForRates <- FLAElapsed[,"std.ref",drop=FALSE]

#check if file already exists; if does, reads it in; if doesn't, create blank dataframe to store calculated rates in
...
#only want to look at bulk incs for this script
CsvNameListBulk <- CsvNameList[grep("stn([0-9]+)-d([0-5])-bulk-([a-z]+)-([0-9a-z])-t([0-9]).csv",CsvNameList)]
...
for (i in 1:length(CsvNameListBulk)) {
    CsvName <- CsvNameListBulk[i]
...
for (j in 1:length(RatesMasterList)) {
    ...
    for (x in 1:length(TimeList[[j]])) {
        ...
        if (all(names(finalR) %in% colnames(FLAMaster)==TRUE)==FALSE) {
            ...
            mess with FlaMaster column names? You did didn't you.")
        }
